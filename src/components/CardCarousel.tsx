...
export default function CardCarousel() {
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      // Accumulate continuous wheel scroll delta for a completely continuous and responsive feel
      // dividing by a tuned factor to yield natural rotation speeds
      const scrollFactor = 0.0015;
      targetProgress.current += e.deltaY * scrollFactor;
      ...
      // Schedule a gentle snap to the nearest whole card when scrolling stops
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      // Smoothly map finger swipe displacement directly to card rotation
      const swipeSensitivity = 0.0045;
      targetProgress.current = touchStartProgress.current + deltaY * swipeSensitivity;

      // Cancel any ongoing snaps while user's finger is down
      if (snapTimeoutRef.current !== null) {
        window.clearTimeout(snapTimeoutRef.current);
      }
    };
    ...
    const handleTouchEnd = () => {
      ...
      // Immediately snap cleanly on release for maximum tactile feedback
      targetProgress.current = Math.round(targetProgress.current);
    };
  ...
  const renderLoop = () => {
    // Smoothly interpolate current progress towards targetProgress
    progress.current += (targetProgress.current - progress.current) * 0.085;
    ...
    // Track dynamic active index state for indicators
    const currentActive = ((roundedIndex % cardCount) + cardCount) % cardCount;
  ...
  return (
    <div
      ref={containerRef}
      className="absolute inset-0 bg-[#000000] text-white flex items-center justify-center overflow-hidden select-none"
    >
      ...
      {/* Elegant Dot Indicators & Scroll Instruction Tip */}
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-3 select-none pointer-events-auto">
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  const currentWrapped = ((current % cardCount) + cardCount) % cardCount;
                  let diff = idx - currentWrapped;
                  if (diff > cardCount / 2) diff -= cardCount;
                  if (diff < -cardCount / 2) diff += cardCount;
                  targetProgress.current = current + diff;
                }}
                className={`h-1.5 rounded-full transition-all duration-500 cursor-pointer ${
                  isActive ? 'w-8 bg-[#e8702a]' : 'w-2 bg-white/35 hover:bg-white/60'
                }`}
        ...
        <div className="text-[9px] sm:text-[10px] tracking-[0.25em] font-mono text-white/40 uppercase animate-pulse flex items-center gap-1.5">
          <span>Scroll wheel / Touch swipe to explore</span>
        </div>
      </div>
    </div>
  );
